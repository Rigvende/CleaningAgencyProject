...
public class ShowCleanersCommand implements ActionCommand {
    ...
    public String execute(HttpServletRequest request) throws CommandException {
        ...
        try {
            List<AbstractEntity> list = service.doService(Attribute.CLEANER.getValue());
            if (!list.isEmpty()) {
                List<User> users = new ArrayList<>();
                for (AbstractEntity entity : list) {
                    users.add((User) entity);
                }
                request.getSession().setAttribute(Attribute.CLEANER_LIST.getValue(), users);
                page = ConfigurationManager.getProperty(Page.PAGE_CLEANERLIST.getValue());
            } else {
