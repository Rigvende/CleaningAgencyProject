...
public class ShowClientsCommand implements ActionCommand {
    ...
    public String execute(HttpServletRequest request) throws CommandException {
        ...
        try {
            List<AbstractEntity> list = service.doService(Attribute.CLIENT.getValue());
            if (!list.isEmpty()) {
                List<User> users = new ArrayList<>();
                for (AbstractEntity entity : list) {
                    users.add((User) entity);
                }
                request.getSession().setAttribute(Attribute.CLIENT_LIST.getValue(), users);
                page = ConfigurationManager.getProperty(Page.PAGE_CLIENTLIST.getValue());
            } else {
