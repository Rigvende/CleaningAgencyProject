...
<form name="changeClientForm" method="POST" action="${pageContext.request.contextPath}/controller">
    ...
    <label>
        <input class="little-width"  type="text" name="discount"
               value="${not empty client.discount ? client.discount : '0.00'}"
    ...
    <label>
        <input class="little-width" type="text" name="notes" value="${client.notes}"/>
    </label></b>

    <div class="little-error">${errorChangeClientMessage}</div>
    ${wrongAction}
    ${nullPage}
    <br/><br/><ctg:remove-attr/>
    <input class="action-submit" type="submit" value="<fmt:message key="button.changeclient"/>"/>
    </div>
</form>
