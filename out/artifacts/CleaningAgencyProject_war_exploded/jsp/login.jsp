...
<form name="loginForm" method="POST" action="${pageContext.request.contextPath}/controller">
    ...
    <label>
        <input type="text" name="login" value="" required oninvalid="this.setCustomValidity('<fmt:message key="message.required"/>')" oninput="setCustomValidity('')"/>
    </label>

        <div style="color: #0c4f5b; font-family: 'Palatino Linotype',serif; font-size: 18px;"><fmt:message key="field.password"/></div>
    <label>
        <input type="password" name="password" value="" required oninvalid="this.setCustomValidity('<fmt:message key="message.required"/>')" oninput="setCustomValidity('')"/>
    </label>
